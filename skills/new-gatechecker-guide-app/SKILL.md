...
このスキルは、GateChecker 向けの新規レース追加を「候補選定 -> race 定義 -> Listing -> 公開 `.iq` -> テスト -> PR -> 独立レビュー -> main マージ」の順で進める。
...
8. `./scripts/run_unit_tests.sh` を実行する。
9. PR 作成、独立レビュー結果の記録、main マージまで実施する。
